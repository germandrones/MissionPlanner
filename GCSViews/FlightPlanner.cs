...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        void saveWPsFast(object sender, ProgressWorkerEventArgs e, object passdata = null)
        {            
            var reqno = 0;
            ...
            for (var a = 0; a < commandlist.Count; a++)
            {
                if (a % 10 == 0 && a != 0)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            e.ErrorMessage = "Upload wps failed " + reqno + " " + Enum.Parse(typeof(MAVLink.MAV_MISSION_RESULT), result.ToString());
                            MainV2.comPort.UnSubscribeToPacketType(sub1);
                            MainV2.comPort.UnSubscribeToPacketType(sub2);
                            return;
                        }
